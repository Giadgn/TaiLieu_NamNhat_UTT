...
void Tong(int a[][100], int n, int m){  // tong các phan tu có tan cùng = 6
    ...
    for (int i = 0; i < n; i++) {
        for(int j = 0; j < m; j++){
            if(a[i][j]%10==6){ // cac so chia cho 10 du 6  
...
//tim max, vi tri cua no
void timax(int a[100][100], int n, int m){
	int max=0;
	for (int  i= 0; i<n; i++){
		for(int j=0; j<m; j++){
			if(a[i][j]>max){
...
void trungbinhcong(int a[100][100], int n, int m){
	int dem=0, tong=0;
	for (int  i= 0; i<n; i++){
		for(int j=0; j<m; j++){
			if(a[i][j]%2!=0&&a[i][j]%3==0){
...
void soHoanHao(int a[100][100], int n, int m) {
    int i, j, k;
    int sum = 0;
